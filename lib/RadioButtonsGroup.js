...
export default class RadioGroup extends Component {
  ...
  componentWillUpdate(){
    if(this.state.radioButtons != this.props.radioButtons)
...
class RadioButton extends Component {
  render() {
    ...
    const fontSize = !this.props.fontSize ? 25 : this.props.fontSize
    const fontColor = !this.props.fontColor ? 'black' : this.props.fontColor
    ...
    return (
      <TouchableOpacity
        ...
        <Text style={[{ alignSelf: 'center', fontSize:fontSize, color:fontColor }, margin]}>{data.label}</Text>
      </TouchableOpacity>
    );
  }
}
